document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.product').forEach(product => {
        const additionalInfoElements = product.querySelectorAll('.additional-info');
        if (additionalInfoElements.length > 1) {
            const lastAdditionalInfoElement = additionalInfoElements[additionalInfoElements.length - 1];
            product.removeChild(lastAdditionalInfoElement);
        }
    });
});
